...
export const projectsZh: Project[] = [
  {
    ...
    attribution:
      "两人团队共创；负责产品逻辑、工作流设计与 AI 辅助开发",
    org: "独立产品",
    period: "2026 – 至今",

    problem:
      "重要承诺散落在邮件、日历与日常对话中。人们经常忘记自己答应过什么、错过应该跟进的事项，也很难把碎片化信息整理成清晰的每日计划。",
...

  {
    ...
    problem:
      "多数健康与健身产品提供的是通用建议，或者只关注卡路里和体重，却很少帮助用户理解饮食、运动、睡眠、压力与坚持程度如何共同影响自己的状态。",

    approach:
      "PulsePilot 允许用户用自然语言记录运动、饮食和感受，再将近期行为与个人历史基线比较，寻找属于用户自己的模式，而不是套用人群平均结论。",

    architecture:
      "自然语言记录 → 结构化行为数据 → 个人滚动基线 → 趋势与关联分析 → 可解释 AI 洞察 → 个性化目标与进度展示。",

    impact:
      "把简单的自我记录转变为个人发现，让用户看到自己的日常习惯与情绪、精力、压力和长期进展之间的关系。",

    lessons:
      "真正的个性化不应该从通用建议开始，而应该从用户自己的历史基线开始。AI 不仅要给出洞察，也必须解释洞察为什么出现。",
...

  {
    ...
    architecture:
      "市场与宏观数据 → 候选标的筛选 → 宏观 / 新闻 / 公司 / 技术分析 → 证据核对 → 多观点比较 → 决策输出 → 盘前计划 → 盘后结果复盘。",
